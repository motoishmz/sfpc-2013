...
void ofApp::setup()
{
	...
	frame_window.width = 12520.; // 42inch in 300dpi
	frame_image.height = frame_window.height / (num_col + 1.);
	frame_image.width = frame_image.height / 3 * 4; // 4 : 3
	padding = frame_image.height / 2.;
	num_row = ceil(num_images / (float)num_row);
	frame_window.height = (num_row * frame_image.width) + (padding * 2);
